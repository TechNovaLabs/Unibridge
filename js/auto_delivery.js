...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    var index_header_link_call = document.getElementById("auto_delivery_header_link_call");
    var index_banner_union_right = document.getElementById("auto_delivery_banner_link_call");
...
document.getElementById('auto_delivery_header_button').addEventListener('click', function() {
    ...
    fetch('/send', {
        ...
        body: JSON.stringify({ name, phone, email, source: 'Страница: Авто доставка' }),  // Добавляем источник
    })
    .then(response => {
        if (!response.ok) {
...
document.getElementById('calculate_button').addEventListener('click', function() {
    ...
    fetch('/send', {
        ...
        body: JSON.stringify({
            ...
            source: 'Страница: Авто доставка. Калькулятор расчета стоимости'
        }),
    })
